...
class CP_Solver():
    ...
    def construct_path(self,solver,y): #this function is to output the path given visited edge
        nextt={}
        for (i,j) in y:
    ...
    def solve(self):
        ...
        for p in range(m):
            for s in range(1,n):
                x[(p,s)]=model.NewIntVar(0,self.data['storage'][p][s],f'x({p},{s})')
        """for i in range(1,n):
        ...
        if status==cp_model.OPTIMAL:
            path=self.construct_path(solver,y)
